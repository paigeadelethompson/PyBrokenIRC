        ...
        if text_template:
            if irc.get_service_bot(source):
                ...
                try:
                    ...
                    log.debug(
                        '(%s) relay_cb_core: Trying to find original sender (server) for %s. serverdata=%s',
                        irc.name,
                        source,
                        args.get('serverdata'))
                    try:
            ...
            if 'channel' in args:
                ...
                log.debug('(%s) relay_clientbot: coersing $channel from %s to %s',
                          irc.name, args['local_channel'], args['channel'])
