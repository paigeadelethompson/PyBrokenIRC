...
def initialize_all(irc):
    ...
    def _initialize_all():
        for chanpair, entrydata in db.items():
            ...
            # Initialize all channels that are relevant to the called network (i.e.
            # channels either hosted there or a relay leaf channels)
            if network == irc.name:
    ...
    with spawnlocks_servers[irc.name]:
        try:
            ...
            if not spawn_if_missing:
                log.debug(
                    '(%s) get_relay_server_sid: %s.relay doesn\'t have a known SID, ignoring.',
                    irc.name,
                    remoteirc.name)
                return
...
def relay_joins(irc, channel, users, ts, targetirc=None, **kwargs):
    ...
    def _relay_joins_loop(irc, remoteirc, channel, users, ts, burst=True):
        ...
        for user in users.copy():
            ...
            assert user in irc.users, "(%s) relay.relay_joins: How is this possible? %r isn't in our user database." % (
                irc.name, user)
        ...
        if queued_users:
            ...
            remoteirc.call_hooks([rsid, 'PYLINK_RELAY_JOIN', {
...
def get_supported_cmodes(irc, remoteirc, channel, modes):
        ...
        for name, m in irc.cmodes.items():
            ...
            if modechar == m:
                ...
                if modechar in irc.prefixmodes:
                    ...
                    # We broke up an acting extban earlier. Now, rewrite it into a new mode by
                    # joining the prefix and data together.
                ...
                if prefix == '+' and (
                        remotechan not in remoteirc.channels or final_modepair in remoteirc.channels[remotechan].modes):
    ...
    log.debug('(%s) relay.get_supported_cmodes: final modelist (sending to %s%s) is %s',
              irc.name, remoteirc.name, remotechan, supported_modes)
    return supported_modes
...
def handle_kick(irc, source, command, args):
    ...
    def _handle_kick_loop(irc, remoteirc, source, command, args):
        ...
        if not is_relay_client(irc, target):
            ...
            log.debug(
                '(%s) relay.handle_kick: target %s is an internal client, going to look up the real user',
                irc.name,
                target)
        ...
        # Propogate the kick!
        if real_kicker:
            log.debug('(%s) relay.handle_kick: Kicking %s from channel %s via %s on behalf of %s/%s',
                      irc.name, real_target, remotechan, real_kicker, kicker, irc.name)
            ...
            rsid = get_relay_server_sid(remoteirc, irc)
            log.debug('(%s) relay.handle_kick: Kicking %s from channel %s via %s on behalf of %s/%s',
                      irc.name, real_target, remotechan, rsid, kicker, irc.name)
    ...
    # Kick was a relay client but sender does not pass CLAIM restrictions.
    # Bounce a rejoin unless we've reached our limit.
    if is_relay_client(irc, target) and not check_claim(irc, channel, kicker):
        if _claim_should_bounce(irc, channel):
            ...
            log.debug(
                '(%s) relay.handle_kick: kicker %s is not opped... We should rejoin the target user %s',
                irc.name,
                kicker,
                real_target)
            ...
            if not remoteirc.has_cap('can-spawn-clients'):
                if numeric in irc.servers and not irc.servers[numeric].has_eob:
                    ...
                    if friendly_modes:
                        ...
                        remoteirc.call_hooks([remotesender, 'RELAY_RAW_MODE', {
                                             'channel': remotechan, 'modes': friendly_modes}])
            ...
            for homechan in origirc.users[realuser[1]].channels.copy():
                ...
                if localchan:
                    ...
                    # If we have no access in a channel, rejoin the target.
                        ...
                        log.debug(
                            '(%s) relay.handle_kill: rejoining target userpair: (%r, %r)',
                            irc.name,
                            modes,
                            realuser)
    ...
    if localchan not in irc.channels or source not in irc.channels[localchan].users:
        ...
        if irc.protoname == 'clientbot':
            ...
            if localchan not in irc.pseudoclient.channels:
                ...
                irc.reply(
                    'Joining %r now to check for op status; please run this command again after I join.' %
                    localchan)
                return
        else:
...
def linkacl(irc, source, args):
    ...
    if cmd == 'list':
        ...
        if whitelist:
            s = 'Whitelisted networks for \x02%s\x02: \x02%s\x02' % (
                channel, ', '.join(entry['allowed_nets']) or '(empty)')
        else:
            s = 'Blocked networks for \x02%s\x02: \x02%s\x02' % (
                channel, ', '.join(entry.get('blocked_nets', set())) or '(empty)')
        irc.reply(s)
        return
    elif cmd == 'whitelist':
