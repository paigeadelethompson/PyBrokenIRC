...
class HybridProtocol(TS6Protocol):
    ...
    def set_server_ban(self, source, duration, user='*', host='*', reason='User banned'):
        ...
        if source not in self.users:
            log.debug(
                '(%s) Forcing KLINE sender to %s as TS6 does not allow KLINEs from servers',
                self.name,
                self.pseudoclient.uid)
    ...
    def handle_capab(self, numeric, command, args):
        ...
        for required_cap in ('SVS', 'EOB', 'HOPS', 'QS', 'TBURST'):
            if required_cap not in caps:
                raise ProtocolError(
                    '%s not found in TS6 capabilities list; this is required! (got %r)' %
                    (required_cap, caps))
    ...
    def handle_uid(self, numeric, command, args):
        ...
        return {'uid': uid, 'ts': ts, 'nick': nick, 'realname': realname,
                'host': host, 'ident': ident, 'ip': ip, 'secure': has_ssl}
