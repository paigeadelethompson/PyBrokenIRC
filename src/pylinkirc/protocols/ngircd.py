...
class NgIRCdProtocol(IRCS2SProtocol):
    ...
    def sjoin(self, server, channel, users, ts=None, modes=set()):
        ...
        if nicks_to_send:
            ...
            for message in utils.wrap_arguments(njoin_prefix, nicks_to_send,
                                                self.S2S_BUFSIZE, separator=',', max_args_per_line=13):
                self.send(message)
    ...
    def handle_nick(self, source, command, args):
        ...
        if len(args) >= 2:
            ...
            # <- :ngircd.midnight.local NICK jlu5 1 ~jlu5 localhost 1 +io :realname
            nick = args[0]
            assert source in self.servers, "Server %r tried to introduce nick %r but isn't in the servers index?" % (
                source, nick)
