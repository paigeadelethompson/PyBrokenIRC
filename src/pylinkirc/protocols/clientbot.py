...
class ClientbotBaseProtocol(PyLinkNetworkCoreWithUtils):
    ...
    def _get_UID(self, nick, ident=None, host=None, spawn_new=False):
        ...
        idsource = self.nick_to_uid(nick, filterfunc=lambda uid: uid ==
                                    self.pseudoclient.uid or not self.is_internal_client(uid))
        ...
        for name in args[-1].strip().split(' '):
            ...
            if 'userhost-in-names' in self.ircv3_caps:
                try:
                    ...
                    log.exception(
                        '(%s) Failed to split hostmask %r from /names reply on %s; args=%s',
                        self.name,
                        nick,
                        channel,
                        args)
    ...
    def handle_part(self, source, command, args):
        ...
        for channel in channels:
            ...
            if is_part_requested:
                log.debug(
                    '(%s) clientbot.handle_part: not forwarding part hook for %s since we requested it',
                    self.name,
                    channel)
        ...
        if notify_channels:
            log.debug(
                '(%s) clientbot.handle_part: returning part hook for %s (original: %s)',
                self.name,
                notify_channels,
                channels)
