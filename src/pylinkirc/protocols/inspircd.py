...
class InspIRCdProtocol(TS6BaseProtocol):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if ircd_target == 'insp20':
            ...
            raise ProtocolError(
                "Unsupported target_version %r: supported values include %s" %
                (ircd_target, self.SUPPORTED_IRCDS))
        log.debug(
            '(%s) inspircd: using protocol version %s for target_version %r',
            self.name,
            self.proto_ver,
            ircd_target)
    ...
    def topic(self, source, target, text):
        ...
        if self.proto_ver >= 1205:
            self._send_with_prefix(source, 'FTOPIC %s %s %s :%s' %
                                   (target, self._channels[target].ts, int(time.time()), text))
        else:
    ...
    def topic_burst(self, source, target, text):
        ...
        if self.proto_ver >= 1205:
            self._send_with_prefix(source, 'FTOPIC %s %s %s %s :%s' %
                                   (target, self._channels[target].ts, topic_ts, servername, text))
        else:
    ...
    def spawn_server(self, name, sid=None, uplink=None, desc=None):
        ...
        if self._endburst_delay:
            t = threading.Thread(
                target=endburstf, name="protocols/inspircd delayed ENDBURST thread for %s@%s" %
                (sid, self.name))
            t.daemon = True
            t.start()
                ...
                if self.proto_ver >= 1205:
                    ...
                    if modetype == 'simple':       # No parameter
                        ...
                        if args[0] != 'CHANMODES':  # This should never happen...
                            log.warning(
                                "(%s) Possible desync? Got a prefix type modepair %r but not for channel modes",
                                self.name,
                                modepair)
                        else:
    ...
    def handle_uid(self, numeric, command, args):
        ...
        return {'uid': uid, 'ts': ts, 'nick': nick, 'realhost': realhost,
                'host': host, 'ident': ident, 'ip': ip, 'secure': None}
