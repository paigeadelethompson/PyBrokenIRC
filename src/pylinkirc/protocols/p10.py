...
class P10Protocol(IRCS2SProtocol):
    ...
    def set_server_ban(self, source, duration, user='*', host='*', reason='User banned'):
        ...
        if duration == 0 or duration > GLINE_MAX_EXPIRE:
            ...
            log.debug('(%s) Lowering GLINE duration on %s@%s from %s to %s',
                      self.name, user, host, duration, GLINE_MAX_EXPIRE)
            duration = GLINE_MAX_EXPIRE

        self._send_with_prefix(source, 'GL * +%s@%s %s %s %s :%s' %
                               (user, host, duration, currtime, currtime + duration, reason))
    ...
    def handle_nick(self, source, command, args):
        ...
        if len(args) > 2:
            ...
            return {'uid': uid, 'ts': ts, 'nick': nick, 'realhost': realhost, 'host': host,
                    'ident': ident, 'ip': ip, 'parse_as': 'UID', 'secure': uobj.ssl}
