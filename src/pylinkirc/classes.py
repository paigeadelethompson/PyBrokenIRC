...
class PyLinkNetworkCore(structures.CamelCaseToSnakeCase):
    ...
    def version(self):
        ...
        """
        fullversion = 'PyLink-%s. %s :[protocol:%s, encoding:%s]' % (
            __version__, self.hostname(), self.protoname, self.encoding)
        return fullversion
    ...
    def _run_autoconnect(self):
        ...
        if autoconnect is not None and autoconnect >= 1:
            log.debug('(%s) _run_autoconnect: Multiplying autoconnect delay %s by %s.',
                      self.name, autoconnect, self.autoconnect_active_multiplier)
                ...
                try:
                    if prefixmodes and mode in self.prefixmodes:
                        ...
                        if prefix == '-':
                            ...
                            self._log_debug_modes(
                                '(%s) parse_modes: checking if +%s %s is in old modes list: %s; existing_casemap=%s',
                                self.name,
                                mode,
                                arg,
                                existing,
                            ...
                            if casefolded_modepair not in existing:
                                ...
                                self._log_debug_modes(
                                    "(%s) parse_modes: ignoring removal of non-existent list mode +%s %s; casefolded_modepair=%s",
                                    self.name,
                                    mode,
                                    arg,
                                    casefolded_modepair)
    ...
    def parse_modes(self, target, args, ignore_missing_args=False):
        ...
        if not is_channel:
            ...
            if target not in self.users:
                self._log_debug_modes(
                    '(%s) Possible desync! Mode target %s is not in the users index.',
                    self.name,
                    target)
                ...
                if their_ts < 750000:
                    if their_ts != 0:  # Sometimes unreal sends SJOIN with 0, don't warn for those
                        if self.serverdata.get('ignore_ts_errors'):
                            ...
                            log.warning(
                                '(%s) Possible desync? Not setting bogus TS %s on channel %s',
                                self.name,
                                their_ts,
                                channel)
                else:
