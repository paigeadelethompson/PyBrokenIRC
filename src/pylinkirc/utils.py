...
def _reset_module_dirs():
    ...
    protocols.__path__ = [protocols.__path__[0]] + \
        [expandpath(path) for path in conf.conf['pylink'].get('protocol_dirs', [])]
...
class ServiceBot():
    ...
    def get_realname(self, irc):
        ...
        """
        sbconf = conf.conf.get(self.name, {})
        return irc.serverdata.get("%s_realname" % self.name) or sbconf.get(
            'realname') or conf.conf['pylink'].get('realname') or self.name
                ...
                if doc:
                    ...
                    if not shortform:
                        ...
                        for linenum, line in enumerate(lines[1:], 1):
                            ...
                            log.debug("_show_command_help: Last line (%s-1=%s): %r",
                                      linenum, linenum - 1, lines[linenum - 1].strip())
